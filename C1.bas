Sub calculate()
    ...
    For i = 0 To UBound(compressed)
        Cells(3 + i, 9) = Round(compressed(i)(UBound(compressed(0))), 6)
    Next i
    Cells(2, 9) = 0
    Cells(2, 10) = Key * compressed(0)(UBound(compressed(0)))
    For i = 0 To UBound(compressed)
      Cells(i + 3, 10) = Cells(i + 3, 8)
    Next i
End Sub
...
Sub reset_area()
    Sheets("Sheet1").Range("I2", "I7").ClearContents
    Sheets("Sheet1").Range("J2", "J7").ClearContents
End Sub
